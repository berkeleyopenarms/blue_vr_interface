...
public class RsStreamTextureRenderer2 : MonoBehaviour
{
    ...
    void Start()
    {
        RsDevice2.Instance.OnStart += OnStartStreaming;
        RsDevice2.Instance.OnStop += OnStopStreaming;
    }
    ...
    {
        RsDevice2.Instance.OnNewSample -= OnNewSampleUnityThread;
        RsDevice2.Instance.OnNewSample -= OnNewSampleThreading;
    ...
    protected virtual void OnStartStreaming(PipelineProfile activeProfile)
    {
        ...
        if (RsDevice2.Instance.processMode == RsDevice2.ProcessMode.UnityThread)
        {
...
public class DefaultStreamAttribute2 : Attribute
{
    public Stream stream;
    public TextureFormat format;

    public DefaultStreamAttribute2(Stream stream)
    {
        this.stream = stream;
    }

    public DefaultStreamAttribute2(Stream stream, TextureFormat format)
    {
        this.stream = stream;
        this.format = format;
    }
